...
<script>
/* ── Restricted Staff IDs (blocked from portal access) ── */
...
function showRestrictionError(input) {
    const existing = document.getElementById('restrictionAlert');
    if (existing) existing.remove();

    const alert = document.createElement('div');
    alert.id = 'restrictionAlert';
    alert.className = 'alert-custom';
    alert.innerHTML = `<i class="fa fa-ban" style="font-size:18px;flex-shrink:0;"></i>
        Access Restricted. This Staff ID is not permitted to use this portal. You are in MPT Group`;
    alert.style.marginTop = '16px';
    alert.style.marginBottom = '0';

    input.classList.add('is-invalid');
    input.closest('form').appendChild(alert);
}

/* ── Clear restriction error ── */
function clearRestrictionError(input) {
    const existing = document.getElementById('restrictionAlert');
    if (existing) existing.remove();
...
document.getElementById('mainLookupForm').addEventListener('submit', function(e) {
    const inputEl  = document.getElementById('staffIdInput');
    const inputVal = inputEl.value.trim().toUpperCase();

    if (RESTRICTED_IDS.has(inputVal)) {
        e.preventDefault();
        showRestrictionError(inputEl);
        return;
    }

    clearRestrictionError(inputEl);
});

/* ── Auto-uppercase + clear error on input ── */
